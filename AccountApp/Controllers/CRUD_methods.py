...
def NameQuery_Decorator_SubLedger(func):
    def wrapper(request, *args, **kwargs):
        ...
        return Response(userLedgers_Serializer(userActivityList, many=True, fields=('id','balance','debit','credit','ledger_id','user_id','description','date') ).data,status=status.HTTP_200_OK)
    return wrapper
...
def RESTcreateLedger(serializer_class, Requestdata):
    try:
        ...
        if serializer.is_valid():
            serializer.save()
            print(f"Serializer was validated for {Requestdata}")
        else:
            print(f"Serializer did not validated {Requestdata}")

    except Exception as e:
        print(f"{serializer_class.__class__} as failed to complete the serialzation due to {e}")
